...
- In the Filebeat-playbook and metricbeat-playbook, edit the file paths for the src location so they copy the config files correctly. Currently the file paths are /etc/ansible/filebeat/filebeat-config.yml and /etc/ansible/metricbeat/metricbeat-config.yml
